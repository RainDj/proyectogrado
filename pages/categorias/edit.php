...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errores)) {
        $sql = "UPDATE categorias
                SET nombre = ?, descripcion = ?, vigencia_tipo = ?, vigencia_cantidad = ?
                WHERE id = ?";
        $stmt = $conexion->prepare($sql);
        $stmt->bind_param(
            'sssii',
    ...
    $vigencia_tipo = $categoria['vigencia_tipo'];
    $vigencia_cant = $categoria['vigencia_cantidad'];
}
                    ...
                    <form method="post">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="vigencia_tipo" id="vigencia_tipo" class="form-select" required>
                                <option value="no_aplica" <?php echo ($vigencia_tipo === 'no_aplica') ? 'selected' : ''; ?>>
                                    No aplica (siempre vigente)
                                </option>
                                <option value="dias" <?php echo ($vigencia_tipo === 'dias') ? 'selected' : ''; ?>>
                                    Por número de días
                                </option>
                            </select>
                        </div>
